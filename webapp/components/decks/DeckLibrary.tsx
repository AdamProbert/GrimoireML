...
import { fetchDecks, createDeck, DeckData } from '../../lib/deckStore';
...
export default function DeckLibrary() {
  const [items, setItems] = useState<DeckData[]>([]);
  ...
  async function handleSubmit() {
    ...
    await createDeck({
      name: name || `Untitled Deck ${items.length + 1}`,
      cards: parsed.cards,
    });
    await load();
    setOpen(false);
